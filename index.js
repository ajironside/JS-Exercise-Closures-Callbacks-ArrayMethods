...
function processSum(numberList, callback) {
    return callback(numberList.reduce(function(a, b) { return a + b; }, 0))
}
...
function getFullNames(runners) {
    const runNames = [];
    runners.forEach(currentNam => {
        runNames.push(`${currentNam.last_name}, ${currentNam.first_name}`)
    });
    return runNames;
}
...
function firstNamesAllCaps(runners) {
    const newNames = [];
    runners.map(capsNames => {
        newNames.push(`${capsNames.first_name.toUpperCase()}`)
    });
    return newNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
    let shirtSizes = runners.filter(function(checkShirt) {
...
function tallyUpDonations(runners) {
    const totalCash = runners.reduce((total, newRunner) => {
        return total += newRunner.donation;
    }, 0);
    return totalCash;
}
